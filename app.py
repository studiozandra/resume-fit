...
st.title('🤖 AI Resume-Desription Compare')
st.markdown('I was made to compare roles to resumes. This app demonstrates how to use OpenAI GPT-3.5 to answer data-related interview questions in a deployed envionment. Remember, always verify AI-generated responses.')
...
def analyze_text(job_description, resume_text):
  ...
  Args:
      resume_text (str): The parsed PDF text from the uploaded file.
      job_description (str): The form input copy-pasted by the user.
  ...
  try:
      ...
      model="gpt-3.5-turbo",
      messages=[
          {"role": "system", "content": F"Using only 3 sentences, tell the candidate some SPECIFIC wording suggestions for resume ( {resume_text} ) improvement so that they can better match the given job description ( {job_description} )." },
          {"role": "user", "content": F"Using only 3 sentences, tell the candidate some SPECIFIC wording suggestions for resume ( {resume_text} ) improvement so that they can better match the given job description ( {job_description} )."}
      ],
      temperature=0.4,            
      )
